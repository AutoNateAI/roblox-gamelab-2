...
**Use this for every investigation's `## How This Connects` section — not ` ```mermaid `.** Mermaid's auto-layout reads as a static print/technical diagram; ` ```graph ` is a hand-laid-out inline SVG that re-themes for free (pure CSS custom properties, no JS re-render needed on dark-mode toggle, unlike Mermaid) and can carry live scenario sliders on top of the same diagram. No external library — `graphBlockHtml()` in `src/pages.mjs` renders the full SVG + slider controls server-side (works with JS off), `public/app.js`'s graph section adds the interactive layer on top.

**Pan, zoom, and drag are automatic on every `graph` block — not something you opt into in the JSON.** The canvas supports drag-to-pan, pinch/wheel/button zoom, and every node is individually draggable (drag to rearrange, e.g. to untangle overlapping labels on a dense diagram) — all pure Pointer Events + native SVG (`<animateMotion>`/`<mpath>` for the flow-dots, so they keep following an edge whose path gets rewritten mid-drag), zero new dependency. This means a `graph` block with no `inputs` still isn't just "a static picture" the way the old wording put it — it's still a pannable/zoomable/draggable canvas, just without a scenario slider on top. Don't add anything to the JSON spec to get this; it's baked into `graphBlockHtml()`/the graph section of `app.js` for every block.
...
- `nodes[].rank`: required, integer, 0-based — which top-to-bottom row the node sits in (rank 0 at the top, flowing downward — reads better than left-to-right, especially on mobile). Nodes sharing a rank sit side by side in that row, auto-centered. This is a manual layout, not a computed one — keep it to what fits legibly in 3-5 ranks, 2-4 nodes per rank.
- `nodes[].label`: use `\n` for manual line breaks (2-3 short lines max) — there's no automatic text wrapping.
- `nodes[].detail` (optional): a sentence or two of evidence/context. A node with `detail` becomes tappable/clickable/keyboard-focusable and gets a small dot indicator; tapping it opens a shared panel below the diagram showing that node's label + detail text. This is the actual "tap a node to see what's behind it" interaction — give every real (non-trivial) node one where you have real evidence to show; skip it on a node that has nothing to add beyond its label.
...
- A `graph` block with no `inputs` still renders fully pan/zoom/drag-interactive (see below) — just without a scenario slider on top, which is fine for pages that don't have a clean two-point anchor to build one from.
