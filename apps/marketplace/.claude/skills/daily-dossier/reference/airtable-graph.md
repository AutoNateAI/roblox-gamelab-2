...
Mirror `regions[]`/`organizations[]`/`systems[]` in `src/data.mjs`. **Read-only for this skill** — search by `Name` via `list_records_for_table` to find the record to link, never create a new one here. If the dossier's question genuinely needs a Region/Organization/System that doesn't exist yet (in Airtable *or* `data.mjs`), that's a bigger scope decision (a new site page) — flag it to Nathan instead of inventing the row.

Current real (non-watchlist) records as of the last check: Region `Southeast Missouri (The Bootheel)`, Organization `Farm Credit Southeast Missouri`, System `Agricultural Finance & Capital`. Re-check live rather than assuming this list is still complete — Regions/Organizations/Systems grow independently of this skill.
