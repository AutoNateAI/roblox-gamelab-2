...
const images = [
  ...
  },
  {
    ...
    size: "1536x1024",
    prompt: `A photorealistic medium shot of the man from the reference headshot sitting beside (not directly across from) a professionally dressed adult (a stand-in for a local business owner client, not based on any real specific individual) at a small round table, both leaning in toward a laptop that sits open at roughly a 45-degree angle on the table between them — angled outward like in real product-demo photography, so its interior screen is genuinely visible to both people AND to the camera at the same time, showing a faint internal dashboard UI with red accents. The camera views the laptop from the same side as the open screen, three-quarter angle, exactly like a real photo of two people looking at a laptop together — never from behind the display. If any part of the laptop's outer lid is visible, it is a plain matte surface with no screen, glow, or UI content; a screen only ever renders on the interior display face that is angled toward the viewer. In a bright modern office/meeting-room setting (not the classroom — a small conference table, glass wall in the background), engaged consulting conversation, warm daylight, documentary editorial style, shallow depth of field. ${STYLE} ${GUARDRAIL} The laptop screen's UI accent color must be a bold red (#c8102e family), not amber. No readable text.`,
  },
];
